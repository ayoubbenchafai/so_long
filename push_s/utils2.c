...
/*   Updated: 2024/03/02 13:06:24 by aben-cha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void update_position(t_data *d, int new_x, int new_y)
{
    ...
    mlx_put_image_to_window(d->mlx_ptr, d->win_ptr, d->img, d->p.x, d->p.y);
    if(d->map[new_y/50][new_x/50] == 'C')
        d->map[new_y/50][new_x/50] = '0';
    if(k == 0 && d->map[new_y/50][new_x/50] == 'E')
        close_window(d);
    if(d->map[new_y/50][new_x/50] != '1' && d->map[new_y/50][new_x/50] != 'E')
    {
        d->p.x = new_x;  
        d->p.y = new_y;  
        ft_putnbr(++i);
